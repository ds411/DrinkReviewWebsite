...
/**
 * Base class that represents a row from the 'drink' table.
 *
 *
 *
 * @package    propel.generator..Base
 */
abstract class Drink implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\DrinkTableMap';
    ...
    /**
     * The value for the style_name field.
     *
     * @var        string
     */
    ...
    /**
     * Initializes internal state of Base\Drink object.
     */
    ...
    /**
     * Compares this with another <code>Drink</code> instance.  If
     * <code>obj</code> is an instance of <code>Drink</code>, delegates to
     * <code>equals(Drink)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Drink The current object, for fluid interface
     */
    ...
    /**
     * Get the [style_name] column value.
     *
     * @return string
     */
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[DrinkTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[DrinkTableMap::COL_NAME] = true;
        }
    ...
    public function setCompanyId($v)
    {
        ...
        if ($this->company_id !== $v) {
            $this->company_id = $v;
            $this->modifiedColumns[DrinkTableMap::COL_COMPANY_ID] = true;
        }
    ...
    /**
     * Set the value of [style_name] column.
     *
     * @param string $v new value
     * @return $this|\Drink The current object (for fluent API support)
     */
    public function setStyleName($v)
    {
        ...
        if ($this->style_name !== $v) {
            $this->style_name = $v;
            $this->modifiedColumns[DrinkTableMap::COL_STYLE_NAME] = true;
        }

        if ($this->aStyle !== null && $this->aStyle->getStyle() !== $v) {
            $this->aStyle = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : DrinkTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : DrinkTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aStyle !== null && $this->style_name !== $this->aStyle->getStyle()) {
            $this->aStyle = null;
        }
    } // ensureConsistency
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildDrinkQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildDrinkQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                DrinkTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aStyle !== null) {
                if ($this->aStyle->isModified() || $this->aStyle->isNew()) {
                    $affectedRows += $this->aStyle->save($con);
                }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[DrinkTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . DrinkTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(DrinkTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(DrinkTableMap::COL_NAME)) {
        ...
        if ($this->isColumnModified(DrinkTableMap::COL_STYLE_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'style_name';
        }

        $sql = sprintf(
            'INSERT INTO drink (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'company_id':
                        ...
                        break;
                    case 'style_name':
                        $stmt->bindValue($identifier, $this->style_name, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    {
        $pos = DrinkTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Drink'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Drink'][$this->hashCode()] = true;
        $keys = DrinkTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
    ...
    {
        $criteria = new Criteria(DrinkTableMap::DATABASE_NAME);

        if ($this->isColumnModified(DrinkTableMap::COL_ID)) {
            $criteria->add(DrinkTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(DrinkTableMap::COL_NAME)) {
            $criteria->add(DrinkTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(DrinkTableMap::COL_PICTURE)) {
        ...
        if ($this->isColumnModified(DrinkTableMap::COL_STYLE_NAME)) {
            $criteria->add(DrinkTableMap::COL_STYLE_NAME, $this->style_name);
        }
    ...
    {
        $criteria = ChildDrinkQuery::create();
        $criteria->add(DrinkTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Drink (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Drink Clone of current object.
     * @throws PropelException
     */
     ...
     * @throws PropelException
     */
    public function setStyle(ChildStyle $v = null)
    {
        if ($v === null) {
            $this->setStyleName(NULL);
        } else {
            $this->setStyleName($v->getStyle());
        }
     ...
     * @throws PropelException
     */
    public function getStyle(ConnectionInterface $con = null)
    {
        if ($this->aStyle === null && (($this->style_name !== "" && $this->style_name !== null))) {
            $this->aStyle = ChildStyleQuery::create()->findPk($this->style_name, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aStyle->addDrinks($this);
             */
        }
    ...
    {
        if (null !== $this->aCompany) {
            $this->aCompany->removeDrink($this);
        }
        if (null !== $this->aStyle) {
            $this->aStyle->removeDrink($this);
        }
        $this->id = null;
        $this->name = null;
        $this->picture = null;
    ...
    {
        return (string) $this->exportTo(DrinkTableMap::DEFAULT_STRING_FORMAT);
    }
